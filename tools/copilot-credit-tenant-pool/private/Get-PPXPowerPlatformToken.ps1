...
#
# Deliberate copy of tools/custom-connector-usage/private/Get-PPXPowerPlatformToken.ps1 -- the PPX
# tools are self-contained by design. Extracting the shared auth code to tools/_shared is a tracked
# follow-up for all three tools.
...
function Get-PPXPowerPlatformToken {
    ...
    .DESCRIPTION
        Every caller in this tool -- Connect-PPXInventoryApi (resourcequery, environment list),
        Get-PPXEnvironmentCreditAllocation (licensing GET) and Set-PPXEnvironmentCreditAllocation
        (licensing PATCH) -- hits the same api.powerplatform.com resource, so the token is acquired
        once by the entry point and passed down, rather than re-running the Az context check per
        environment. It is also handed down as a -TokenFactory scriptblock so a long multi-environment
        run can refresh on an HTTP 401.
