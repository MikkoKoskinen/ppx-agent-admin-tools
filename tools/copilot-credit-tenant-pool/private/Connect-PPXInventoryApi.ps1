...
# tools/custom-connector-usage/private/Connect-PPXInventoryApi.ps1 (itself copied from
# tools/agent-governance-baseline). The PPX tools are independent and self-contained by design. This
# tool uses it for exactly one query -- listing every environment in the tenant so -AllEnvironments
# has a target set and every row carries a display name / type / environment group. The caller passes
# -Clauses; the delegated token is acquired by Get-PPXPowerPlatformToken or passed in; a -TokenFactory
# lets a long run refresh an expired token; a non-progress guard stops a query whose skipToken never
# terminates. Extracting the shared code to tools/_shared is a tracked follow-up for all three tools.
