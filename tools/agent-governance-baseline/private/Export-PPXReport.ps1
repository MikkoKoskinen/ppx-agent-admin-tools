function Export-PPXReport {
    ...
    $limitationsLines += @(
        ...
        ''
        'This report pass (Inventory + Graph owner resolution; no DLP/connector-catalog enrichment):'
        '- OwnerName, OwnerUPN, OwnerAccountStatus are resolved via Microsoft Graph (Resolve-PPXOwnerIdentity) for non-managed agents. OwnerAccountStatus of "NotFound" means the ownerId no longer resolves to a directory object (deleted/cross-tenant); "GraphError" means the Graph lookup itself failed this run; "NotApplicable" means either IsManagedAgent = True (OwnerName = "Microsoft (managed agent)", never sent to Graph) or a non-managed agent with no ownerId at all (OwnerName = "(no owner)"). The raw OwnerId GUID is always included regardless.'
